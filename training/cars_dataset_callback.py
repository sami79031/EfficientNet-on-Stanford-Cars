...
def log_dictionary(dictionary, trainer):
    for k, v in dictionary.items():
        try:
            if hasattr(trainer.logger.experiment, 'log_metric'):
                trainer.logger.experiment.log_metric(k, v)
            else:
                # For TensorBoard, we can't log arbitrary metrics during training start
...
class StanfordCarsDatasetCallback(Callback):
    ...
    def on_train_start(self, trainer, pl_module):
        ...
        if trainer.logger is not None:
            # Check if it's a Neptune logger or TensorBoard logger
            if hasattr(trainer.logger.experiment, 'log_metric'):
                ...
                # For TensorBoard logger, just log basic info
                log.info(f'Trial info: {self.trial_info.get_trial_info()}')
